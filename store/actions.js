...
export default {
  async PULL_BRANCHS ({state, commit}) {
    await axios.get(state.currentIP + '/api')
      .then(res => {
        let result = res.data
  ...
  async PULL_COURSE_FROM_BRANCH_ID ({state, commit, dispatch}, branch_id) {
    ...
    if (isCheck == false) {
        await axios.get(state.currentIP + '/api/getcourse/' + branch_id)
      .then(res => {
        let result = res.data
  ...
  async PULL_COURSE_FROM_COURSE_ID ({commit, state, dispatch, route}, courseId) {
    ...
    if(haveCourse == true) {
      ...
      if(isCheck == false) {
        ...
        await axios.get(state.currentIP + '/api/course/' + courseId)
        .then (res => {
          let result = res.data
          result.map(rs => {
            let {course_id,user_id,branch_id,subject,code,price,des,cover,ts,lastUpdate,fname,lname,user_img,email,facebook,twitter,youtube, five, four, three, two, one, avg, length} = rs
  ...
  async PUSH_COURSE ({state, commit}, data) {
    await axios.post(state.currentIP + '/api/insertcourse', data)
  },
  ADD_COURSE_PURCHASED ({state, commit}, payload) {
    commit('addCoursePurchased', [payload.course_id])
    axios.post(state.currentIP + '/api/insertuserpurchase', payload)
  },
  ADD_COURSE_FAVORITE ({state, commit}, payload) {
    ...
    axios.post(state.currentIP + '/api/insert_course_fav', data)
  ...
  REMOVE_COURSE_FAVORITE ({state, commit}, payload) {
    ...
    axios.post(state.currentIP + '/api/delete_course_fav', data)
    commit('removeCourseFavorite', payload)
  },
  UPDATE_PROFILE ({state, commit}, payload) {
    commit('updateProfile', payload)
    axios.post(state.currentIP + '/api/updateuser', payload)
  },
  ...
  ADD_COURSE_CONTENT ({commit, state, dispatch}, payload) {
    ...
    delete payload["data"]
    axios.post(state.currentIP + '/api/insertcoursecontent', payload)
    .then (res => {
      let result = res.data
      ...
      axios.post(state.currentIP + '/api/upload/' + result.content_id, filesData)
      .then((response) => {
  ...
  async PULL_POPULAR_COURSE_HOME ({commit, state}) {
    ...
    if (state.popularCourseHome.length == 0) {
      await state.branchs.map(data => {
         axios.get(state.currentIP + '/api/popularcourse/' + data.branch_id)
        .then (res => {
          let result = res.data
          result.map(str => {
  ...
  PULL_POPULAR_COURSE_INDEX ({commit, state}) {
    if (state.popularCourseIndex.length == 0) {
      axios.get(state.currentIP + '/api/popularcourse')
      .then (res => {
        let result = res.data
        result.map(str => {
          let {user_id,fname,lname,user_img,sex,birthday,email,facebook,twitter,youtube ,course_id,branch_id,subject,code,price,des,cover,ts,coupon,lastUpdate, five, four, three, two, one, avg, length} = str
  ...
  async PULL_USER_PURCHASED ({commit, state}, course_id) {
    ...
    if (isCheck == false) {
      await axios.get(state.currentIP + '/api/userpurchased/' + course_id)
      .then (res => {
        let result = res.data
  ...
  async PULL_USER ({commit, state}, user_id) {
    ...
    if (isCheck == false) {
      await axios.get(state.currentIP + '/api/user/' + user_id)
      .then(res => {
        let result = res.data
  ...
  ADD_REVIEW ({commit, state}, payload) {
    ...
    axios.post(state.currentIP + '/api/insertreview', payload)
  },
  PULL_COURSE_REVIEW ({commit, state, getters}, courseId) {
    let isCheck = false
    state.courseReview.find(cr => cr.course_id == courseId ? isCheck = true : '')
    if (isCheck == false) {
        axios.get(state.currentIP + '/api/get_review_course_order_ts/' + courseId)
        .then (res => {
          let result = res.data
          if (result !== undefined) {
  ...
  async PULL_COURSE_CONTENT ({commit, state}, course_id) {
    ...
    if (isCheck == false) {
      await axios.get(state.currentIP + '/api/get_course_content/' + course_id)
      .then (res => {
        content_id = res.data
        new Vue().$socket.emit('subscribe', content_id)
      })
      content_id.map(con => {
        axios.get(state.currentIP + '/api/get_course_content_file/' + con.content_id)
        .then(res => {
          let result = res.data
          let file = []
  ...
  async PULL_COURSE_QA ({commit, state}, course_id) {
    ...
    if (isCheck == false) {
      await axios.get(state.currentIP + '/api/get_course_q/' + course_id)
      .then(res => {
        qa = res.data
        commit('setCourseQA', res.data)
      })
      await qa.map((q) => {
        axios.get(state.currentIP + '/api/get_course_q_comment/' + q.q_id)
        .then(res => {
          let result = {
            q_id: q.q_id,
            comment: res.data
  ...
  ADD_COURSE_QA ({commit, state}, payload) {
    axios.post(state.currentIP + '/api/insertcourse_q', payload)
    .then (res => {
      let result = res.data
  ...
  async PULL_COURSE_ANNO ({state,commit}, course_id) {
    ...
    if (isCheck == false) {
      await axios.get(state.currentIP + '/api/get_course_announce/' + course_id)
      .then(res => {
        qa = res.data
        commit('setCourseAnno', res.data)
      })
      await qa.map((q)  => {
        axios.get(state.currentIP + '/api/get_course_announce_comment/' + q.annou_id)
        .then(res => {
            let result = {
              annou_id: q.annou_id,
              comment: res.data
  ...
  ADD_COURSE_ANNO ({commit, state}, payload) {
    axios.post(state.currentIP + '/api/insertcourse_announce/', payload)
    .then (res=> {
      let result = res.data
  ...
  ADD_COURSE_ANNO_COMMENT ({state, commit}, payload) {
    axios.post(state.currentIP + '/api/insertcourse_announce_comment/', payload)
  },
  // async PULL_USER_ONLINE ({state, commit}, course_id) {
  //   let new_course_id = course_id +  11111
  //   let result
  //     await axios.get(state.currentIP + '/api/get_userinroom/' + course_id)
  //     .then (res => {
  //       result = res.data
  ...
  USER_ONLINE ({state, commit}, payload) {
    // axios.post(state.currentIP + '/api/insertusertoroom/', payload)
  },
  USER_OFFLINE ({state, commit}, payload) {
    // axios.post(state.currentIP + '/api/deleteuserfromroom/', payload)
  },
  FETCH_COURSE_REVIEW ({commit, state}) {
    console.log('FETCH_COURSE_REVIEW')
    if (state.isCheckReview !== true) {
        axios.get(state.currentIP + '/api/get_review_course_user/' + state.profile.user_id)
      .then (res => {
        let result = res.data
  ...
  async FETCH_COURSE_PURCHASED ({commit, state, dispatch}) {
    if (state.isCoursePurchased !== true ) {
      axios.get(state.currentIP + '/api/get_all_userpurchased/' + state.profile.user_id)
      .then (res => {
        let result = res.data
  ...
  async FETCH_COURSE_CREATED ({commit, state}) {
    ...
    if (state.isCourseCreated !== true ) {
      axios.get(state.currentIP + '/api/get_all_userowner/' + state.profile.user_id)
      .then (res => {
  ...
  async FETCH_COURSE_FAVORITE ({commit, state}) {
    ...
    if (state.isCourseFavorite !== true ) {
      axios.get(state.currentIP + '/api/get_all_userfavorite/' + state.profile.user_id)
      .then (res => {
  ...
  FETCH_NOTIFICATION ({state, commit}, payload) {
    axios.get(state.currentIP + '/api/get_notification_type1')
    .then (res => {
      let result = res.data
      commit('addNotification', result)
    })
    axios.get(state.currentIP + '/api/get_notification_type2/' + payload)
    .then (res => {
      let result = res.data
      commit('addNotification', result)
    })
  ...
  //   axios.post(state.currentIP + '/api/insertnotification', payload)
  // },
  ...
  PULL_USER_DATA ({state, commit}, user_id) {
    ...
    if (isCheck == false) {
      axios.get(state.currentIP + '/api/user/' + user_id)
      .then (res => {
        let result = res.data
  ...
  PULL_USER_OWNER ({state, commit}, user_id) {
    ...
    if (isCheck == false) {
      axios.get(state.currentIP + '/api/get_all_userowner/' + user_id)
      .then (res => {
        let result = res.data
  ...
  PULL_USER_STUDENT ({state, commit}, user_id) {
    ...
    if (isCheck == false) {
      axios.get(state.currentIP + '/api/get_all_my_user/' + user_id)
      .then (res => {
        let result = res.data
  ...
  PULL_USER_REVIEW ({state, commit}, user_id) {
    ...
    if (isCheck == false) {
      axios.get(state.currentIP + '/api/get_all_my_review/' + user_id)
      .then (res => {
        let result = res.data
        result[0].user_id = user_id
  ...
  PULL_USER_PURCHASE ({state, commit}, user_id) {
    ...
    if (isCheck == false) {
      axios.get(state.currentIP + '/api/get_all_userpurchased/' + user_id)
      .then (res => {
        let result = res.data
